...
/**
 * @类 名 称：storageHandler2062
 * @类 描 述：上传监测指标核查数据
 * @功能描述：上传监测指标核查数据
 * @创建作者：quanhongsheng
 */
@Component("storageHandler2062")
public class StorageHandler2062 implements StorageHandler {

	private static Logger logger = LoggerFactory.getLogger(StorageHandler2062.class);
	...
	public void storage(JSONObject data) {
		...
		try {
			...
			List<Msg2062VO> params = new ArrayList<Msg2062VO>();

			for (String json : list) {

				params.add(JSON.parseObject(json, Msg2062VO.class));

			}

			if (list.size() > 0) {
				ctlMsgService.insert2062ByBatch(params);
			}
		...
		try {
			if (datas != null && datas.size() > 0) {
				...
				for (int i = 0; i < group; i++) {
					...
					ctlMsgService.insert2062ByBatch((List<Msg2062VO>) datas.subList(fromIndex, toIndex));
					long e = System.currentTimeMillis();
					logger.info(String.format("T2062入库：数量：%s 耗时：  %s ", (toIndex - fromIndex), (e - b)));
				}
			}
		} catch (Exception e) {
