...
import com.zeei.das.aas.alarm.OnlineAlarm;
import com.zeei.das.aas.alarm.custom.GKCancelAlarm;
...
class StartEvent implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		if (event.getApplicationContext().getParent() == null) {
			...
			logger.info("启动业务程序！");
			// 初始化站点在线告警处理类
			BeanUtil.getBean("onlineAlarm", OnlineAlarm.class).alarmHandler(null);

			// 工况停产消警处理
			BeanUtil.getBean("gkCancelAlarm", GKCancelAlarm.class).alarmHandler();
